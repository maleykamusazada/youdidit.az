...
(function () {
  var current = 0;
  var revealed = false;
  var marks = new Array(QUESTIONS.length).fill(null); // true = bildim, false = bilmedim
  ...
  function render() {
    ...
    if (!revealed) {
      html += '<button type="button" id="reveal-btn" class="btn btn-primary" style="width:100%;">Cavabı göstər</button>';
    } else {
      html += '<div class="logic-answer"><strong>Cavab:</strong> ' + item.a + '</div>';
      html += '<div class="quiz-mark-row">';
      html += '<button type="button" id="got-btn" class="btn ' + (marks[current] === true ? 'btn-primary' : 'btn-outline') + '">✓ Bildim</button>';
      html += '<button type="button" id="missed-btn" class="btn ' + (marks[current] === false ? 'btn-primary' : 'btn-outline') + '">✗ Bilmədim</button>';
      html += '</div>';
    }

    html += '<div class="quiz-nav">';
    html += '<button type="button" id="prev-btn" class="btn btn-outline"' + (current === 0 ? ' disabled' : '') + '>Geri</button>';
    html += '<button type="button" id="next-btn" class="btn btn-primary"' + (marks[current] === null ? ' disabled' : '') + '>' + (current === QUESTIONS.length - 1 ? 'Bitir' : 'Növbəti') + '</button>';
    html += '</div>';
    ...
    if (revealBtn) {
      revealBtn.addEventListener("click", function () {
        revealed = true;
        marks[current] = marks[current] === null ? undefined : marks[current];
        render();
        document.getElementById("got-btn") && document.getElementById("got-btn").focus();
      });
    }
  ...
  function finish() {
    ...
    for (var i = 0; i < QUESTIONS.length; i++) {
      var d = QUESTIONS[i].d;
      byDiff[d].total++;
      if (marks[i] === true) { score++; byDiff[d].ok++; }
    }
    ...
    document.getElementById("restart-btn").addEventListener("click", function () {
      current = 0;
      marks = new Array(QUESTIONS.length).fill(null);
