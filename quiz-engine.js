...
// exam-data-*.js faylları ilə işləyir: QUIZ_META = {...}, QUESTIONS = [{ q, options: [...], a: cavabIndeksi }, ...]
...
(function () {
  ...
  function render() {
    ...
    for (var i = 0; i < item.options.length; i++) {
      ...
      html += '<label class="quiz-option"><input type="radio" name="quiz-answer" value="' + i + '"' + checked + '> <span>' + item.options[i] + '</span></label>';
    }
    html += '</div>';
    html += '<div class="quiz-nav">';
    html += '<button type="button" id="prev-btn" class="btn btn-outline"' + (current === 0 ? ' disabled' : '') + '>Geri</button>';
    html += '<button type="button" id="next-btn" class="btn btn-primary"' + (answers[current] === null ? ' disabled' : '') + '>' + (current === QUESTIONS.length - 1 ? 'Bitir' : 'Növbəti') + '</button>';
    html += '</div>';
    root.innerHTML = html;
  ...
  function sendResultByEmail(pct, score) {
    ...
    data.append("Test", (typeof QUIZ_META !== "undefined" && QUIZ_META.title) || document.title);
  ...
  function downloadCertificate(pct, score) {
    ...
    var title = (typeof QUIZ_META !== "undefined" && QUIZ_META.title) || "";
    ...
    function draw() {
      ...
      wrapText(ctx, title + " testini tamamlayıb.", W / 2, 470, W - 260, 34);
